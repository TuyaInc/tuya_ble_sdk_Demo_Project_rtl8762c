...
static void tuya_ota_data_req(uint8_t*recv_data,uint32_t recv_len)
{
    ...
    else
    {
        if((current_package==0)&&(dfu_settings.write_offset=0))
        {
            ...
            if (dfu_control_point.p.start_dfu.ic_type != 0x05 || dfu_control_point.p.start_dfu.signature != AppPatch  \
                    || (dfu_control_point.p.start_dfu.image_length+IMG_HEADER_SIZE) != dfu_settings.file_info.firmware_file_length)
            {
                state = 4;
        ...
        else
        {
            ...
            if (ota_tmp_buf_used_size == 2048 || dfu_settings.write_offset + ota_tmp_buf_used_size == dfu_settings.file_info.firmware_file_length)
            {
                unlock_flash_all();
                if (sil_dfu_update(AppPatch, dfu_settings.write_offset, ota_tmp_buf_used_size,(uint32_t *)TempBufferHead) == 0)
                {
                    lock_flash();
                    ...
                    dfu_settings.progress.firmware_image_crc_last = crc32_compute(TempBufferHead, ota_tmp_buf_used_size, &dfu_settings.progress.firmware_image_crc_last);
